...
num_px = train_set_x_orig.shape[1]  # 训练集、测试集图片的高度和宽度均为64*64
...
print("训练集降低后的维数：" + str(train_set_x_flatten.shape))
print("训练集_标签的维数量: " + str(train_set_y.shape))
print("测试集降低后的维数：" + str(test_set_x_flatten.shape))
print("测试集_标签的维数: " + str(test_set_y.shape))
...
def Initialize_with_zeros(dim):
    ...
    b = 0
    # 使用断言函数确保数据的正确性
    assert (w.shape == (dim, 1)) # 确保w的维数是（dim， 1）
